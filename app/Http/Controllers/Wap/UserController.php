...
class UserController extends BaseController
{
    ...
    public function my(Request $request){
        ...
        $UserId =$request->session()->get('UserId');
           ...
           return $this->WapShowTemplate("user.my");
    ...
    public function loginloglist(Request $request){
        ...
        if($request->ajax()){
            ...
            return $this->WapShowTemplate("user.memberlogs",["status"=>0,"list"=>$list,"pagesize"=>$pagesize]);
        }
    ...
    public function edit(Request $request){
        ...
        if($request->ajax()){
           ...
           if($EditMember){
            ...
            return $this->WapShowTemplate("user.edit");
        }
    ...
    public function shiming(Request $request){
        ...
        if($request->ajax()){
           ...
           if($EditMember){
               ...
               if($EditMember->card==''){
                 ...
                 $card=  Member::where("card",$request->card)->first();
                   ...
                   if(Cache::get('CouponGiveOn')=='开启') {
                       /****实名赠送优惠券****/
                       $RealnameCoupon = Cache::get('RealnameCoupon');
                       \App\Couponsgrant::PushCoupon($RealnameCoupon, $EditMember->id, 3);
                   }
            ...
            return $this->WapShowTemplate("user.shiming");
        }
    ...
    public function bank(Request $request){
        ...
        if($request->ajax()){
            ...
            if($request->bankname==''){
                return ["status"=>1,"msg"=>"请选择银行"];
            }
            if($request->bankrealname==''){
                return ["status"=>1,"msg"=>"请填写姓名"];
            }
            if($request->bankcode==''){
                return ["status"=>1,"msg"=>"请填写卡号"];
            }
            if($request->bankaddress==''){
                return ["status"=>1,"msg"=>"请填写开户银行"];
            }
           ...
           if($EditMember){
            ...
            return $this->WapShowTemplate("user.bank");
        }
    ...
    public function certification(Request $request){
            ...
            return $this->WapShowTemplate("user.certification");
    ...
    public function phone(Request $request){
        ...
        if($request->ajax()){
           ...
           if($EditMember){
            ...
            return $this->WapShowTemplate("user.phone");
        }
    ...
    public function security(Request $request){
        ...
        if($request->ajax()){
           ...
           if($EditMember){
            ...
            return $this->WapShowTemplate("user.security");
        }
    ...
    public function password(Request $request){
        ...
        if($request->ajax()){
           ...
           if($EditMember){
            ...
            return $this->WapShowTemplate("user.password");
        }
...
*/
            ...
            return $this->WapShowTemplate("user.paypwd");
        }
    ...
    public function retrieve(Request $request){
        ...
        if($request->ajax()){
           ...
           if($EditMember){
            ...
            return $this->WapShowTemplate("user.retrieve");
        }
    ...
    public function msglist(Request $request){
        ...
        if($request->ajax()){
            ...
            return $this->WapShowTemplate("user.msglist",["status"=>0,"list"=>$list,"pagesize"=>$pagesize]);
        }
    ...
    public function products(Request $request){
        ...
        if($request->ajax()){
            ...
            return $this->WapShowTemplate("user.products",["status"=>0,"list"=>$list,"pagesize"=>$pagesize]);
        }
...
$ip='127.0.0.8';
            ...
            $NewProductbuy->userid=$this->Member->id;
            $NewProductbuy->username=$this->Member->username;
            ...
            $NewProductbuy->level=$this->Member->level;
    ...
    public function nowToMoney(Request $request){
        ...
        if($pwdPay==''){
            echo json_encode(array('status'=>0,'msg'=>"请您先输入的交易密码"));
            die();
        }


        if(!$amountPay){
            echo json_encode(array('status'=>0,'msg'=>"投资金额有误"));
            die();
        }
            ...
            if($isft == 0){
               $Productbuy= Productbuy::where("productid",$idPay)->where("userid",$this->Member->id)->count();
                if($Productbuy>=$futoucishu){
                    echo json_encode(array('status'=>0,'msg'=>"抱歉，此项目只允许投".$futoucishu."次。"));
                    die();
                }
            }else if($isft == 1){
                $Productbuy= Productbuy::where("productid",$idPay)->where("userid",$this->Member->id)->count();
        ...
        if($Product->tzzt == -1){
            ...
            /**现金券查询**/
            $Couponsgrant= Couponsgrant::where("id",$request->cashcoupon)->where("uid",$this->Member->id)->where("type",1)->where("status",1)->first();

            /**加息券查询**/
            $CouponsgrantRate= Couponsgrant::where("id",$request->ratecoupon)->where("uid",$this->Member->id)->where("type",2)->where("status",1)->first();


               if($Couponsgrant){

                   if($Couponsgrant->money>=$amountPay){
                       echo json_encode(array('status' => 0, 'msg' => "现金券金额不能大于等于投资金额"));
                       die();
                   }

                   if (floatval($Mamount) < floatval($amountPay-$Couponsgrant->money)) {
...
if($Product->zsid !=0&&$Product->zsje !=0){
...
/**/
        ...
        if($CouponsgrantRate){
            ...
            $log=[
                ...
                "username"=>$this->Member->username,
                "money"=>0,
                "notice"=>"加入项目(".$Product->title.")(加息:".$CouponsgrantRate->money.")(-)",
                "type"=>"加入项目",
                "status"=>"-",
                "yuanamount"=>$Mamount,
            ...
            $log=[
                ...
                "username"=>$this->Member->username,
                "money"=>0,
                "notice"=> "加入项目-使用(".$CouponsgrantRate->name.")(".$Product->title.")(-)",
                "type"=>"加息券",
                "status"=>"-",
            ...
            $msg=[
                "userid"=>$this->Member->id,
                "username"=>$this->Member->username,
                "title"=>"使用(".$CouponsgrantRate->name.")",
                "content"=>"成功加入项目(".$Product->title.")使用(".$CouponsgrantRate->name.")",
                "from_name"=>"系统通知",
                "types"=>"加息券",
            ];
            \App\Membermsg::Send($msg);
            ...
            /**标记加息券已使用**/
            $CouponsgrantRate->status=2;
            $CouponsgrantRate->save();
        ...
        if($Couponsgrant){
            /**使用现金券**/
            $YHQamountPay=  $amountPay-$Couponsgrant->money;

            $BuyMember->decrement('amount',$YHQamountPay);
            $log=[
                "userid"=>$this->Member->id,
                "username"=>$this->Member->username,
                "money"=>$YHQamountPay,
                "notice"=>"加入项目(".$Product->title.")(余额:".$YHQamountPay."现金券:".$Couponsgrant->money.")(-)",
                "type"=>"加入项目",
                "status"=>"-",
                "yuanamount"=>$Mamount,
                "houamount"=>$BuyMember->amount,
            ...
            $log=[
                "userid"=>$this->Member->id,
                "username"=>$this->Member->username,
                "money"=>$Couponsgrant->money,
                "notice"=> "加入项目-使用(".$Couponsgrant->name.")(".$Product->title.")(-)",
                "type"=>"现金券",
                "status"=>"-",
                "yuanamount"=>$BuyMember->amount,
                "houamount"=>$BuyMember->amount,
            ...
            $msg=[
                "userid"=>$this->Member->id,
                "username"=>$this->Member->username,
                "title"=>"使用(".$Couponsgrant->name.")",
                "content"=>"成功加入项目(".$Product->title.")使用(".$Couponsgrant->name.")",
                "from_name"=>"系统通知",
                "types"=>"现金券",
            ];
            \App\Membermsg::Send($msg);
            ...
            if($hkfs>4){
                ...
                $sendDay_count=round($zhouqi/$hkfs);
           ...
           $NewProductbuy= new Productbuy();
            ...
            /**更新标记现金优惠券cashcoupon**/
            if($Couponsgrant){
                $NewProductbuy->cashcouponid=$Couponsgrant->id;
                $NewProductbuy->cashcoupontype=$Couponsgrant->type;
                $NewProductbuy->cashcouponmoney=$Couponsgrant->money;
                $NewProductbuy->cashcouponname=$Couponsgrant->name;

                $Couponsgrant->usebuyid=$NewProductbuy->id;
                $NewProductbuy->save();
                $Couponsgrant->save();
            ...
            /**更新标记加息券**/
            if($CouponsgrantRate){
                $NewProductbuy->ratecouponid=$CouponsgrantRate->id;
                $NewProductbuy->ratecoupontype=$CouponsgrantRate->type;
                $NewProductbuy->ratecouponmoney=$CouponsgrantRate->money;
                $NewProductbuy->ratecouponname=$CouponsgrantRate->name;

                $CouponsgrantRate->usebuyid=$NewProductbuy->id;
                $NewProductbuy->save();
                $CouponsgrantRate->save();
...
$dj=$ShangjiaMember->level;
//print_r($ShangjiaMember->level);




 $levels= Memberlevel::where("id","=",$dj)->orderBy("id","desc")->first();
...
$djfy=$levels->djfy;
                    ...
                    if(isset($Proxxtcbl[$recent->id])){
                        $recentpercent= $Proxxtcbl[$recent->id]+$djfy;
                    }else{
                        $recentpercent= $recent->percent+$djfy;
                    }
                    //分成钱数
...
//exit;
                ...
                //$sqltz = "select sum(amount) as zetz from  {$db_prefix}project_buy where username = '".$buyman."'";
                //$rstz = $db->get_one($sqltz);
                //用户等级
                $levels= Memberlevel::where("inte","<=",$member_level_amount)->orderBy("id","desc")->first();

                //$sql = "SELECT id FROM {$db_prefix}member_level WHERE inte <= '{$rstz['zetz']}' ORDER BY id DESC limit 1";
                //$level = $db->get_one($sql);
                //$sql_que = "update  {$db_prefix}member set  level = '{$level['id']}'  where  username='".$buyman."'";
                //$db->query($sql_que);
    ...
    public function QrCodeBg(Request $request){
        ...
        $logo= public_path('uploads/'.Cache::get("waplogo"));
    ...
    public function lotterys(Request $request){
        ...
        if($request->ajax()){
            ...
            return $this->WapShowTemplate("user.lotterys");
        }
