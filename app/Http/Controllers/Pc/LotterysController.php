...
class LotterysController extends BaseController
{
    ...
    public function index(Request $request){
        ...
        return $this->PcShowTemplate("lotterys.index");
    ...
    public function winlist(Request $request){
        ...
        foreach($list as $item){
            echo '<li>会员:' ,substr($item->moneylog_user,0,4)."****".substr($item->moneylog_user,8,3) , ' ' , $item->moneylog_notice , ' ' , $item->created_at , '<l/i>';
        }
    ...
    public function click(Request $request){
        ...
        $levelwheels= Memberlevel::where("id",$Member->level)->value('wheels');


        if($clicks>=$levelwheels){
            //return ["state"=>0,"msg"=>"今天抽奖次数已用完"];
            echo 'cjgo(0,"今天抽奖次数已用完")';
            exit;
        }
        ...
        */
        ...
        foreach ($LotteryPeizhi as $lott){
            ...
            $prizeprize[]=$lott->prize;
            $prizeanimate[]=$lott->animate;
        }
        ...
        echo 'cjgo(' , $prizeanimate[$winIndex] , ',"' , $prizeprize[$winIndex], '")';
