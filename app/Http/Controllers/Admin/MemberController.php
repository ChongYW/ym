            ...
            $list = DB::table($this->table)
                ...
                ->where(function ($query) {
                    ...
                    if(isset($_REQUEST['s_status']) && $_REQUEST['s_status']=='2'){
                        $query->whereDate("created_at",Carbon::now()->format("Y-m-d"));
                    }else if(isset($_REQUEST['s_status']) && $_REQUEST['s_status']=='1'){
                        $query->where("updated_at",">",Carbon::now()->addSeconds(-600));
                    }
                })
                ->orderBy("id","desc")
            ...
            if($list){
                ...
                //
                   ...
                   // $item->moneys=$item->recharges-$item->withdrawals;
                    ...
                    $item->moneys=DB::table("moneylog")->where("moneylog_status","+")->whereIn("moneylog_userid",$item->tuiguangren)
