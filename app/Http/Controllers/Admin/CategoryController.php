...
class CategoryController extends BaseController
{
    ...
    public function __construct(Request $request)
    {
        ...
        $WapTemplate= explode("|",DB::table("setings")->where("keyname","WapTemplate")->value("valuelist"));
        $PcTemplate= explode("|",DB::table("setings")->where("keyname","PcTemplate")->value("valuelist"));
        $Templates=array_merge($WapTemplate,$PcTemplate);
        view()->share("templates",$Templates);
        view()->share("PcTemplates",$PcTemplate);
        view()->share("WapTemplates",$WapTemplate);
           ...
           ->where(function ($query) {
            ...
            ->where(function ($query) {
                ...
                if(isset($_REQUEST['s_mtype']) && $_REQUEST['s_mtype']!=''){
                   // $s_model[]=[$this->table.".templates","=",$_REQUEST['s_mtype']];
                    $query->whereRaw("FIND_IN_SET(?,templates)",$_REQUEST['s_mtype']);
                }
    ...
    public function store(Request $request){
        ...
        if($request->isMethod("post")){
            ...
            if(count($request->input('templates'))<1){
                return response()->json([
                    "msg"=>"至少选择一个模板风格","status"=>1
                ]);
            }
            if($request->input('model')!='links'){
               $conut= $this->Model::where("templates",$request->input('templates'))->where("links",$request->input('links'))->count();
               if($conut>0){
                   if($request->ajax()){
                       return response()->json([
            ...
            //$Model->thumb_url2 = $request->input('thumb_url2');
            ...
            $Model->templates = implode(",",$request->input('templates'));
            $Model->save();
    ...
    public function update(Request $request)
    {
        if($request->isMethod("post")){
            ...
            if($request->input('model')!='links'){
                $conut= $this->Model::where("templates",$request->input('templates'))->where("links",$request->input('links'))->whereNotIn("id",[$request->input('id')])->count();
                if($conut>0){
    ...
    public function atindex(Request $request)
    {
        if($request->isMethod("post")){
            ...
            if(isset($data['isshow'])){
                $Model->isshow = $request->input('isshow');
            }

            if(isset($data['morec'])){
                $Model->morec = $request->input('morec');
            }
