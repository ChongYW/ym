...
class Seal extends Model
{
    ...
    //构造方法
    public function __construct($str = '', $rad = 75, $rmwidth = 6, $strad = 24, $stang = 0, $crang = 0, $fsize = 16, $inrad = 0,$strMaxLeng=20)
    {
        $this->sealString = empty($str) ? '印章测试字符串' : $str;
        $this->strMaxLeng = $strMaxLeng;
        $this->sealRadius = $rad;
        ...
        $this->font = public_path('uploads/font/msyhbd.ttf');
        $this->fontSize = $fsize;
    ...
    private function drawString()
    {
        ...
        //相关计量
        $this->charRadius = $this->sealRadius - $this->rimWidth - $this->fontSize- $this->fontSize/2; //字符串半径
        ...
        imagettftext($this->img, $this->fontSize+5, 0,$this->sealRadius/2+$this->fontSize/2,  $this->sealRadius*2-$this->fontSize-$this->fontSize/2, $this->backGround, $this->font, '合同章');
    }
