...
class Memberwithdrawal extends Model
{
    ...
    protected function AddWithdrawal($userid,$Withdrawalamount){
       ...
       $withdrawalbalance= Cache::get('withdrawalbalance');

        if($Member->amount<$Withdrawalamount){
            return ["status"=>1,"msg"=>"帐户余额不足"];
        }

        if(($Member->amount-$withdrawalbalance)<$Withdrawalamount){
            return ["status"=>1,"msg"=>"帐户余额必须保留".$withdrawalbalance."元"];
        }
    ...
    protected function CancelWithdrawal($id){
        ...
        if($Model->status==0){
            ...
            if(Cache::get('AutoSendMsg')=='自动') {
                \App\Sendmobile::SendUid($Model->userid, 'txsb', $Model->amount);//成功短信通知
                $Model->sendsms=1;
                $Model->save();
            }
    ...
    protected function ConfirmWithdrawal($id){
        ...
        if($Model->status==0){
            ...
            if(Cache::get('AutoSendMsg')=='自动') {
                \App\Sendmobile::SendUid($Model->userid, 'txcg', $Model->amount);//成功短信通知
                $Model->sendsms=1;
                $Model->save();
            }
...
*/
    ...
    protected function WithdrawalAmount($userid,$Amounts){
        ...
        if($TixianAmounts>$UserMoneys){
            ...
            return ["status"=>1,"msg"=>"提现失败,投资收益还差".sprintf("%.2f",($UserMoneys-$TixianAmounts)>0?($UserMoneys-$TixianAmounts):-($UserMoneys-$TixianAmounts)).",目前可以提现金额:".sprintf("%.2f",($UserMoneys-$WithdrawalAmounts)>0?($UserMoneys-$WithdrawalAmounts):0)];
        }else{
            return ["status"=>0,"msg"=>"操作成功"];
        }
