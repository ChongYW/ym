...
$(function(){
    $("div.select_box").eq(0).find(".select_option").click(function(){
        if($(".select_box").eq(0).find(".selected").html()=="天标"){
            var $select_box= $(".select_box").eq(1);
            $select_box.find("div.select_showbox").html("一次性还");
            $select_box.find("li").eq(0).siblings().hide();
            $(".tyepname").html("天");
            status = 1;
        }else{
            $(".select_box").eq(1).find("li").show();
            $(".tyepname").html("天");
        }
    })
});
...
function jssy() {
    ...
    if (flag) {
        ...
        var html='';
        html+='<li>';
        html+='<dl class="c_dd_titlecpm dd_titlecpm">';
        html+='<dd>收款日期</dd>';
        html+='<dd>收款金额</dd>';
        html+='<dd>收回本金</dd>';
        html+='<dd>收回利息</dd>';
        ...
        if($(".select_showbox").val() == "1"){//到期还本还息
            ...
            html2+='<dl>';
            html2+='<dd>总结</dd>';
            html2+='<dd>'+(parseFloat(periods*apr*account/100)+parseFloat(account))+'</dd>';
            html2+='<dd>'+(account)+'</dd>';
            html2+='<dd>'+(periods*apr*account/100)+'</dd>';
            html2+='<dd class="c_cpmlastdd" ></dd>';
            html2+='</dl>';
            html2+='</li>';
            html2+='<li class="c_libottomcpm libottomcpm">实际总收益：<span>￥'+(periods*apr*account/100)+'元</span></li>';
        }
            ...
            html2+='<dl>';
            html2+='<dd>总结</dd>';
            html2+='<dd>'+(parseFloat(periods*apr*account/100*7)+parseFloat(account))+'</dd>';
            html2+='<dd>'+(account)+'</dd>';
            html2+='<dd>'+(periods*apr*account/100*7)+'</dd>';
            html2+='<dd class="c_cpmlastdd" ></dd>';
            html2+='</dl>';
            html2+='</li>';
            html2+='<li class="c_libottomcpm libottomcpm">实际总收益：<span>￥'+(periods*apr*account/100*7)+'元</span></li>';
        }
            ...
            html2+='<dl>';
            html2+='<dd>总结</dd>';
            html2+='<dd>'+(parseFloat(periods*apr*account/100*30)+parseFloat(account))+'</dd>';
            html2+='<dd>'+(account)+'</dd>';
            html2+='<dd>'+(periods*apr*account/100*30)+'</dd>';
            html2+='<dd class="c_cpmlastdd" ></dd>';
            html2+='</dl>';
            html2+='</li>';
            html2+='<li class="c_libottomcpm libottomcpm">实际总收益：<span>￥'+(periods*apr*account/100*30)+'元</span></li>';
        }
            ...
            for(var i = 1;i<= periods;i++){
                ...
                lx += parseFloat(apr*account/100);//计算利息
                html+='<li>';
                html+='<dl class="c_dd_titlecpm dd_titlecpm">';
                html+='<dd>'+GetDateStr(parseInt(i))+'</dd>';
                if(i==periods){
...
$(function() {
    ...
    $(window).resize(function() {
        $(".calculatorCpmBox").css({
            left : ($(window).width() - 874) / 2 + "px",
            top : ($(window).height() - 454) / 2 + "px"
        });
        $(".calculatorCpmBottom").css({
