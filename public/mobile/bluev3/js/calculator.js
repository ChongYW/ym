...
$().ready(function(){
	$(".c_cpmbuttons").click(function() {
	    ...
	    if (flag) {
	        ...
	        }else if($(".select_showbox").val() == "7"){//按时返息,到期还本
	        	var pd = periods *24;
	            for(var i = 1;i<= pd;i++){
	                html+='<li>';
	                html+='<dl class="c_dd_titlecpm">';
	                html+='<dd>'+GetHourStr(parseInt(i/24),i)+'</dd>';
	                if(i==periods){
	                    html+='<dd>'+((parseFloat((apr*account)/(100*24))+parseFloat(account)).toFixed(3))+'</dd>';
	                    html+='<dd>'+(account)+'</dd>';
	                }
	                else{
	                    html+='<dd>'+(((apr*account)/(100*24)).toFixed(3))+'</dd>';
	                    html+='<dd>0</dd>';
	                }
	                html+='<dd>'+(((apr*account)/(100*24)).toFixed(3))+'</dd>';
	                if(i!=periods) {
...
function GetHourStr(AddDayCount,hours) {
    var dd = new Date();
