...
@section('form')
    ...
    <script id="demo" type="text/html">
            ...
            <tr>
                ...
                <td><% item.tzzt==1?'已投满':item.tzzt=='-1'?'待发布':'投资中' %></td>

                <td>

                    <%# if(item.status==0){ %>
                    <input type="checkbox"   lay-skin="switch" lay-filter="switchTest-status" lay-text="显示|隐藏" id="<% item.id %>">
                    <%# }else{ %>
                    <input type="checkbox"  checked  lay-skin="switch" lay-filter="switchTest-status" lay-text="显示|隐藏" id="<% item.id %>">
...
/*        form.on('radio(authid)', function(data){
        ...
        form.on('switch(switchTest-status)', function(data){
            var id=data.elem.id;
