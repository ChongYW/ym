...
<html class="v_scrollbar"><head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
    ...
    <title>项目投资协议</title>
    <style type="text/css">
        @if(Cache::get("OfficialSeal")=='电子章')
            .seal{ background: url("{{route('CompanySeal')}}") no-repeat; width: 150px; height: 150px;position: absolute;top:-50px;left:90px;}
        @else
            .seal{ background: url("/uploads/{{Cache::get('offiseal').'?t='.time()}}") no-repeat; width: 150px; height: 150px;position: absolute;top:30px;left:90px;}
        @endif


        @if(Cache::get("OfficialSeal")=='电子章')
            .sealgs{ background: url("{{route('Seal',['id'=>$Pro->id])}}") no-repeat; width: 150px; height: 150px;position: absolute;top:-50px;left:130px;}
        @else
            @if($Pro->insuranceseal!='')
                .sealgs{ background: url("{{$Pro->insuranceseal}}") no-repeat; width: 150px; height: 150px;position: absolute;top:-50px;left:130px;}
        @else
                .sealgs{ background: url("{{route('Seal',['id'=>$Pro->id])}}") no-repeat; width: 150px; height: 150px;position: absolute;top:-50px;left:130px;}
        @endif
        @endif
...
<div class="wrap">
    <div class="ylb-text">
        ...
        <p>本投资协议（“本协议”）由以下三方于 <strong><u><?php echo date('Y年m月d日',strtotime($ProBuy->useritem_time)); ?></u></strong></p>
        <p>甲方（管理方）：<strong><u><?php echo \Cache::get('CompanyLong'); ?></u></strong></p>
        <p>乙方（投资方）：<strong> <u><?php echo $Mb->realname!=''?$Mb->realname:'*****'; ?></u></strong></p>
        <p>丙方（担保方）：<strong> <u><?php  echo $Pro->bljg;?></u></strong></p>
        <p>银行账号：<strong>
                <u>
        ...
        <table cellpadding="0" cellspacing="0" class="table">
            ...
            <tr>
                ...
                <td>
                    @if($Pro->hkfs == 4)
                        {{\App\Product::Benefit($Pro->id,$ProBuy->amount,$ProBuy->ratecouponmoney)}}
                    @else
                        {{round($Pro->shijian*$ProBuy->amount*($Pro->jyrsy+$ProBuy->ratecouponmoney)*0.01,2)}}
                    @endif
                </td>
                <td><?php  echo $ProBuy->amount;?></td>
            </tr>
        ...
        <p>既乙方通过由甲方运营管理的<?php echo \Cache::get('CompanyLong'); ?>平台进行其公司运营操作的产品项目投资意愿、双方根据平等、自愿的原则，达成本协议如下：</p>
        ...
        <table cellpadding="0" cellspacing="0"  class="table" width="1099" height="130">
            ...
            <tr>
                ...
                <td width="266"><u>
                        ...
                        echo date("Y-m-d H:i:s",strtotime("$d+1hour"));
                        ?></u>  - <u><?php echo date("Y-m-d H:i:s",strtotime("$d+ {$Pro->shijian} hour"));  ?>
                        <?php }else{?>
                        <?php
                        $d = $ProBuy->useritem_time;
                        echo date("Y-m-d",strtotime("$d+1day"));
                        ?></u>  - <u><?php echo date("Y-m-d",strtotime("$d+ {$Pro->shijian} day"));  ?>
                        <?php } ?>
                    </u>
                </td>
            </tr>
            <tr><td colspan="2">项目投资要求</td><td colspan="4"><p>{{$Pro->ketouinfo}}</p></td></tr>
            </tbody>
        </table>

        <h2>二、 投资流程</h2>
        <p>2.1 本协议成立：甲方首先在<?php echo \Cache::get('CompanyLong'); ?>平台上发布相关的投资产品项目收益，乙方按照<?php echo \Cache::get('CompanyLong'); ?>的规则，通过在<?php echo \Cache::get('CompanyLong'); ?>网站上相关的投资产品项目收益，按钮确认
            投资时，本协议下所约定的关系即成立，本协议的签署关系
            同时成立。</p>
        <P>2.2 投资资金冻结：乙方点击“立即投资”按钮即视为其已经向甲方发出不可撤销的授权指令，授权甲方全权处理和操作本次的投资金额（“甲方<?php echo \Cache::get('CompanyLong'); ?>账户”）中的冻结金额等同于本协议
        ...
        <p>5.3 发生下列任何一项或几项情形的， (1) 甲方、乙方
            任何财产遭受没收、征用、查封、扣押、冻结等可能影响其履约能力的不利事件，且不能及时提供有效补救措施的；(2) 提前终止本协议；(3) 采取法律、法规以及本协议约定的其它救济措施。</p>

        <h2>六、 本协议未经乙方事先书面（包括但不限于电子邮件等方式）同意，甲方不得将本协议项下的任何权利义务转让给任何第三方。</h2>
        <h2>七、 其它</h2>
        <p>7.1 本协议在下述条件全部满足时生效：</p>
        <p>（1）甲方、乙方、丙方在本协议上签字盖章、且乙方通过<?php echo \Cache::get('CompanyLong'); ?>以网络在线点击：立即投资的方式签订；<br/>
            （2）自甲方发布的投资项目起，乙方所投项目资金全部在<?php echo \Cache::get('CompanyLong'); ?>账户中本金将被冻结直至项目周期结束。</p>


        <p>7.2 本协议的任何修改、补充均须以<?php echo \Cache::get('CompanyLong'); ?>平台电子文本形式作出。</p>
        <p>7.3 三方均确认，本协议的签订、生效和履行以不违反法律为前提。如果本协议中的任何一条或多条违反适用的法律，则该条将被视为无效，但该无效条款并不影响本协议其它条款的效力。</p>
        <p>7.4 如果双方在本协议履行过程中发生任何争议，应友好协商解决；如协商不成，则提交所在地的人民法院进行诉讼。</p>
        <p>7.5 甲方保留与本协议有关的书面文件或电子信息。</p>


        <div oncontextmenu="window.event.returnValue=false" style="top:100px;height: 280px;width:450px;line-height: 30px;position: relative;">
                <span>甲方：<?php echo \Cache::get('CompanyLong'); ?>
                <br/>
                    <?php
                    $d = $ProBuy->useritem_time;
                    echo date("Y年m月d日",strtotime("$d"));
                    ?>
                </span>
            <div class="seal" oncontextmenu="window.event.returnValue=false">


            </div>
        </div>

        <div oncontextmenu="window.event.returnValue=false" style="top:-30px;height: 100px;width:450px;line-height: 30px;position: relative;">
            <span>乙方：<?php echo $Mb->realname!=''?$Mb->realname:'*****'; ?></span><br/>
            <span><?php echo $Mb->card!=''?'身份证号码：'.$Mb->card:''; ?></span>
            <div class="" oncontextmenu="window.event.returnValue=false">
        ...
        <div oncontextmenu="window.event.returnValue=false" style="height: 280px;width:450px;line-height: 30px;position: relative;">
                <span>丙方：<?php  echo $Pro->bljg;?>
                <br/>
                    <?php
                    $d = $ProBuy->useritem_time;
                    echo date("Y年m月d日",strtotime("$d"));
            ...
            <div class="sealgs" oncontextmenu="window.event.returnValue=false">
    ...
    <script type="text/javascript">
                ...
                @if(Cache::get("OfficialSeal")=='电子章')
        var srl ="{{route('CompanySeal').'?t='.time()}}";
        var url = 'url("'+srl+'")';
        $(".seal").css({"background-image":url,"background-size":"150px 150px"});
                @else

        var srl ="/uploads/{{Cache::get('offiseal').'?t='.time()}}";
        var url = 'url("'+srl+'")';
        $(".seal").css({"background-image":url,"background-size":"150px 150px"});
                ...
                @if(Cache::get("OfficialSeal")=='电子章')
        var srl2 ="{{route('Seal',['id'=>$Pro->id])}}";
        var url = 'url("'+srl2+'")';
        $(".sealgs").css({"background-image":url,"background-size":"150px 150px"});
                @else
                @if($Pro->insuranceseal!='')


        var srl2 ="{{$Pro->insuranceseal}}";
        var url = 'url("'+srl2+'")';
        $(".sealgs").css({"background-image":url,"background-size":"150px 150px"});
