            ...
            <div class="scroller" >
                <ul class="clearfix">
                    <li style="margin-left: 0px; margin-right: 0px;">
                        <a href="{{route("products")}}">全部项目</a>
                    </li>

                    @if($ProductsCategoryList)
                        @foreach($ProductsCategoryList as $Ckey=>$CategoryList)
                            @if($request->links==$CategoryList->links)
                                <?php $Ceckid=$Ckey+1;?>
                            @endif
                            @if($CategoryList->model=='links')

                                <li @if($request->links==$CategoryList->links) class="cur" @endif style="margin-left: 0px; margin-right: 0px;">
                                    <a href="{{$CategoryList->links}}">{{$CategoryList->name}}</a>
                                </li>

                            @else

                                <li @if($request->links==$CategoryList->links) class="cur" @endif style="margin-left: 0px; margin-right: 0px;">
                                    <a href="{{route($CategoryList->model.".links",["links"=>$CategoryList->links])}}">{{$CategoryList->name}}</a>
                                </li>
                            @endif
                                ...
                                <ul>
                                    ...
                                    <li>
                                        ...
                                        @if($Pro->hkfs == 4)
                                            ...
                                            总收益:<span
                                                    class="red">{{\App\Product::Benefit($Pro->id,$Pro->qtje)}} 元</span>
                                        ...
                                        @else
                                            ...
                                            @endif:<span
                                                    class="red">{{$Pro->jyrsy*$Pro->qtje/100}} 元</span>
                                        @endif
